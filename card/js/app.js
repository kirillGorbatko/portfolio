...
let pageSlider = new Swiper('.page', {
		...
		on: {
			...
			init: function () {
				menuSlider();
				setScrollType();
				wrapper.classList.add('_loaded_hiding');
					window.setTimeout(function () {
						wrapper.classList.add('_loaded');
						wrapper.classList.remove('_loaded_hiding');
					}, 1500);
			},
	...
	for (let index = 0; index < pageSlider.slides.length; index++) {
		...
		if (pageSlideContent) {
			...
			if (pageSlideContentHeight > window.innerHeight) {
				...
				pageSlider.params.freeModeMinimumVelocity = 0.2;
				pageSlider.params.freeModeMomentumRatio = 0.7;
				pageSlider.params.freeModeMomentumBounceRatio = 0.7;
				pageSlider.params.freeModeMomentumVelocityRatio = 0.7;
...
function setScrollReviews(){
	if (!isMobile.any()) {
		...
		reviewsBlock.addEventListener('mousemove' || 'click', function(e){
			if (e.target.parentElement.className === "item-screen__review")
